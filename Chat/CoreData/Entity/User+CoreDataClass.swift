...
public class User: NSManagedObject {
    ...
    static func insertUser(with userID: String, in context: NSManagedObjectContext) -> User? {
        guard let user = NSEntityDescription.insertNewObject(
            forEntityName: "User",
            into: context
            ) as? User
        else { return nil }
    ...
    static func fetctRequestUser(with userID: String, model: NSManagedObjectModel) -> NSFetchRequest<User>? {
        ...
        guard let fetchRequest = model.fetchRequestFromTemplate(
            ...
            substitutionVariables: ["userID": userID]
            ) as? NSFetchRequest<User> else {
    ...
    static func getUsers(online: Bool, context: NSManagedObjectContext) -> [User] {
        guard let model = context.persistentStoreCoordinator?.managedObjectModel else {
            assert(false)
            return []
        }
    ...
    static func fetctRequestOnlineUsers(online: Bool, model: NSManagedObjectModel) -> NSFetchRequest<User>? {
        ...
        guard let fetchRequest = model.fetchRequestFromTemplate(
            withName: templateName,
            substitutionVariables: ["isOnline": online]
            ) as? NSFetchRequest<User>
        else {
            assert(false, "No template witf name \(templateName)!")
            return nil
        }
